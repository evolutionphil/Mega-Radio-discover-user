...
-   **Recently Played:** Discover page shows a "Recently Played" horizontal section at the top (before genres) when the user has listened to stations. Uses `recentlyPlayedService` (localStorage, max 6 stations). Section auto-hides when empty.
-   **For You (Personalized Recommendations):** Discover page shows a "For You" section after Recently Played, based on listening habits. `recommendationService` tracks genre/country preferences in localStorage (50 entries max). Requires 3+ plays to activate. Fetches stations by top genres.
-   **Sleep Timer:** `SleepTimerContext` provides 15min, 30min, 1hr, 2hr options. Configurable in Settings page. Auto-pauses playback when timer expires. Shows remaining time (💤 MM:SS) on RadioPlaying page.
-   **Stream Error UI:** `GlobalPlayerContext` exposes `streamError`, `retryCurrentStation`, `clearStreamError`. When all 3 retries fail, RadioPlaying shows a pink error banner with "Retry" button (focus index 100). Error clears on new station play or successful retry.
-   **Accessibility:** `AccessibilityContext` provides High Contrast mode (increased opacity/contrast via CSS class `.high-contrast`) and Large Text mode (15% font size increase via `.large-text`). Toggleable in Settings page, persisted to localStorage.
-   **Ambient Mode:** RadioPlaying auto-activates after 3 minutes idle (using `useIdleDetection` hook) while playing. Shows floating gradient orbs and clock display. Dismisses on any key press. CSS animations for OLED-safe slow movement (20-30s cycles).
